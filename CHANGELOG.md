...
- **Inflated integrity/availability scores and an over-broad fabrication keyword no longer corrupt otherwise valid reports.** `I:H` now requires state-change evidence from the exploitation proof itself rather than hypothetical takeover language in the description, while `A:H` requires proof of code execution, a crash/outage, or resource exhaustion. An arbitrary file read therefore stays at the evidence-backed `C:H/I:N/A:N` impact instead of becoming critical by assertion alone. Valid proof that explains an artifact is “not a placeholder” is no longer rejected merely for containing the word `placeholder`; explicit simulated/placeholder findings remain blocked.
